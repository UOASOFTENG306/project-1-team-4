...
public class GetFavouritesByCategoryUseCase {
    private static GetFavouritesByCategoryUseCase instance;
    private UserRepository userRepository;

    private GetFavouritesByCategoryUseCase(){
        userRepository = userRepository.getInstance();
    }
    public static GetFavouritesByCategoryUseCase getInstance() {
        if (instance == null){
            instance = new GetFavouritesByCategoryUseCase();
        }
        return instance;
    }

    public LiveData<List<Restaurant>> getFavouritesByCategory(List<Category> categoryList) {
                ...
                if (map != null && map.containsKey("favourites")) {
                    ...
                    if (innerMap != null && innerMap.containsKey("favouriteRestaurants")) {
                        ...
                        for (Map<String, Object> itemMap : array) {
                            Map<String, Object> nestedField = (Map<String, Object>) itemMap.get("category");
                            String categoryType = (String) nestedField.get("categoryType");
                            for(Category category: categoryList){
                                if(categoryType.equals(category.getCategoryType())){
